...
module.exports = {
	...
	aliases: ["skip", "stop"],
	async execute(client, message, args, cmd, guild, Discord) {
		const vC = message.member.voice.channel;
		if (!vC)
			return message.reply("You need to be in a voice channel dumbass!");

		const permissions = vC.permissionsFor(message.client.user);
		if (!permissions.has("CONNECT") || !permissions.has("SPEAK"))
			return message.channel.send("You dont have the permssss");
		...
		if (cmd === "play") {
			...
			if (!args.length)
				return message.channel.send(
					"Please enter some keywords to help me search"
				);
			let song = {};
		...
		} else if (cmd === "skip") skip_song(message, server_queue, guild);
...
const video_player = async (guild, song, message) => {
	...
	if (!song) {
		setTimeout(() => song_queue.connection.destroy(), 30000);
	...
	await song_queue.text_channel.send(
		`<:youtube:910636711875317771> Now playing : ***${song.title}***`
	);
};

const skip_song = (message, server_queue) => {
	console.log("rentrer in");
	if (!message.member.voice.channel)
		return message.channel.send(
			"You need to be in a voice channel dumbass!"
		);
	if (!server_queue.songs[1])
		return message.channel.send("The queue is empty");
	server_queue.songs.shift();
	guild = server_queue.guildId;
